...
contract HashedTimeLock {
    ...
    modifier withdrawable(bytes32 _txId) {
        ...
        require(transactions[_txId].timelock > now, "timelock time must be in the future");
        _;
    }
    modifier refundable(bytes32 _txId) {
        ...
        require(transactions[_txId].withdrawn == false, "this transaction already withdrawn");
        require(transactions[_txId].timelock <= now, "timelock not yet expired");
        _;
    }
    ...
    event LogWithdraw(bytes32 indexed txId);
    event LogRefund(bytes32 indexed txId);
